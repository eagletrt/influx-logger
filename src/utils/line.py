...
class Line:
    ...
    def __init__(self, measurement: Any, tags: dict[str, str],
                 fields: dict[str, Any], timestamp: int) -> None:
    ...
    @staticmethod
    def from_object(obj: dict[str, Any], measurement: Any,
                    tags: dict[str, str]) -> "Line":
        '''
        ...
        fields: dict[str, Any] = {
            k: v
            for k, v in obj.items() if k not in TIMESTAMP_KEYS
        }
        if len(fields) == 0:
    ...
    def to_point(
        ...
        normalized_timestamp = self._normalize_timestamp(
            self.timestamp, timestamp_precision)
